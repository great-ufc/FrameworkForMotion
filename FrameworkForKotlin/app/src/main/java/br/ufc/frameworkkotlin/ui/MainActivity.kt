...
import br.ufc.frameworkkotlin.model.cloudConnection.KnowledgeDownload
...
import br.ufc.frameworkkotlin.model.mapek.analysis.FeatureExtraction
...
import br.ufc.frameworkkotlin.model.utils.FileConvertFunctions
...
class MainActivity : BaseActivity(),SensorEventListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //---for local Test ---//
        //---Comment if you will not use///


        datas+= ValuesSensor (typeSensor.ACC.name, ValuesACC(0.1,0.4,1.2))
        datas+= ValuesSensor (typeSensor.ACC.name, ValuesACC(0.2,0.5,1.5))
        datas+= ValuesSensor (typeSensor.ACC.name, ValuesACC(0.3,0.6,1.6))

        assetManager = getAssets()
        val fKGraph = this.assetManager.open("KnowledgeGraph.xml").bufferedReader().readText()
        var features = FeatureExtraction(this).featureDataExtraction()
        knowledgeRepresentation =  KnowledgeDownload.readXMLGraph(fKGraph,features)
/*
        for (models in knowledgeRepresentation!!.edgeModelsFinalStatus) {
            if(models.vModel.modelName.split(".")[1].contains("tflite")){
               // val outFile = File("file.tflite");
                if(models.vModel.modelName.split(".")[1].contains("tflite")) {
                    val outFile = FileConvertFunctions.inputStreamToFile(
                        this.assets.open(models.vModel.modelName),
                        getExternalFilesDir(null)!!.absolutePath+"/"+models.vModel.modelName
                    )
                    val pathModelFile = outFile.absolutePath//getExternalFilesDir(null)!!.absolutePath +
                          //  "/saved_model/" + models.vModel.modelName
                    /*

                    val inputStream: FileInputStream = FileInputStream(outFile)
                    val declaredLength: Long = outFile.length()
                    val fileChannel = inputStream.channel
                    val loadmodel =
                        fileChannel.map(FileChannel.MapMode.READ_ONLY, 0, declaredLength)
                    */
                    var interpreter = Interpreter(KnowledgeDownload.loadModelFile(pathModelFile, this.assetManager!!)!!, null)
