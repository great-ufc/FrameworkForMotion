...
package br.ufc.frameworkkotlin.model.cloudConnection
...
class KnowledgeDownload {
    companion object{
        ...
        fun update(mutableList: MutableList<VerticeFeature>, context: BaseActivity): KnowledgeRepresentation{
            ...
            try {
                knowledgeGraph =
                    downloadClassificationGraph(Constants.LAST_OPTIMIZE_GRAPH_ADDRESS, context);
        ...
        fun downloadClassificationGraph(url:String, context: BaseActivity):String {
        ...
        //Executa um download
        fun download(url: URL, outputFileName: String, context: BaseActivity) {
            val base = context.baseDirectory
        ...
        fun download2(url:String, outputFileName: String, context: BaseActivity) {
            val client = OkHttpClient()
            val policy = StrictMode.ThreadPolicy.Builder().permitAll().build()
            StrictMode.setThreadPolicy(policy)
            val request = Request.Builder()
            ...
            client.newCall(request).execute().use { response ->
                ...
                val fileW = BufferedWriter(FileWriter(File(context.baseDirectory+outputFileName)))
                fileW.write(response.body!!.string())
                fileW.close()
            }
        }
