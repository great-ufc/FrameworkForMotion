...
class FeatureExtraction(val context: BaseActivity)  {
    ...
    fun getFeatures(smvValues:MutableList<Any>?): MutableList<VerticeFeature>{
         var features = mutableListOf<VerticeFeature>();

        ///////////////VALORES FEATURES/////////////////////

        //----Valores Accelerometer-----//
        features+= VerticeFeature(FeatureFunctions.Mean(smvValues as MutableList<Double>), ValuesFeatures(typeSensor.ACC.name,"mean"));//média
        features+= VerticeFeature(FeatureFunctions.Max(smvValues as MutableList<Double>), ValuesFeatures(typeSensor.ACC.name,"max"));//Valor Máximo
        features+= VerticeFeature(FeatureFunctions.Min(smvValues as MutableList<Double>), ValuesFeatures(typeSensor.ACC.name,"min"));//Valor Mínimo
        features+= VerticeFeature(FeatureFunctions.STD(smvValues as MutableList<Double>), ValuesFeatures(typeSensor.ACC.name,"std"));//Desvio Padrão
        features+= VerticeFeature(FeatureFunctions.Kurtosis(smvValues as MutableList<Double>), ValuesFeatures(typeSensor.ACC.name,"kurrtosis"));//Kurtosis
        features+= VerticeFeature(FeatureFunctions.Skewness(smvValues as MutableList<Double>), ValuesFeatures(typeSensor.ACC.name,"skewness"));//Skewness
        features+= VerticeFeature(FeatureFunctions.Entropy(smvValues as MutableList<Double>), ValuesFeatures(typeSensor.ACC.name,"entropy"));//Entropy
        features+= VerticeFeature(FeatureFunctions.MAD(smvValues as MutableList<Double>), ValuesFeatures(typeSensor.ACC.name,"MAD"));//Mean Absolute Devination
        features+= VerticeFeature(FeatureFunctions.IQR(smvValues as MutableList<Double>), ValuesFeatures(typeSensor.ACC.name,"IQR"));//IQR
        /*
